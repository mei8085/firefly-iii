...
> 补充内容：月末日期修正逻辑、动态规则触发器、历史/当日/未来边界行为、口径统一修订
...
private function needsWarning(Bill $bill, string $field): bool
{
    ...
    // 计算与今日的天数差（Carbon::diffInDays 返回绝对值）
    $diff = $this->getDiff($bill, $field);
...
到期提醒的触发逻辑是 **「配置周期命中就触发，且受偏好开关拦截」**，具体表现为：

1.  **触发条件**：只要 `diff`（日期差的绝对值）在配置列表 `[90, 30, 14, 7, 0]` 中，就会触发事件
2.  **拦截条件**：在监听器中检查 `notification_bill_reminder` 偏好，`false` 则拦截不发送通知
3.  **时间范围**：无内置时间范围限制，无论目标日期是过去、现在还是未来，只要绝对值匹配就触发
4.  **去重机制**：无去重，每次 Cron 运行满足条件就触发（依赖 Cron 12小时间隔控制频率）

**完整边界行为表**（今日日期：2026-05-19）：

| 日期场景 | 目标日期（end_date） | diff 值 | 是否触发事件 | 是否发送通知 | 说明 |
|---------|---------------------|---------|-------------|-------------|------|
| **未来日期** | 2026-08-17（90天后） | 90 | ✅ 触发 | 偏好开启则发送 | 提前90天提醒 |
| **未来日期** | 2026-06-18（30天后） | 30 | ✅ 触发 | 偏好开启则发送 | 提前30天提醒 |
| **未来日期** | 2026-06-02（14天后） | 14 | ✅ 触发 | 偏好开启则发送 | 提前14天提醒 |
| **未来日期** | 2026-05-26（7天后） | 7 | ✅ 触发 | 偏好开启则发送 | 提前7天提醒 |
| **当日** | 2026-05-19（今天） | 0 | ✅ 触发 | 偏好开启则发送 | 到期日当天提醒 |
| **历史日期** | 2026-05-12（7天前） | 7 | ✅ 触发 | 偏好开启则发送 | 过期7天后也触发（因绝对值匹配） |
| **历史日期** | 2026-05-05（14天前） | 14 | ✅ 触发 | 偏好开启则发送 | 过期14天后也触发（因绝对值匹配） |
| **历史日期** | 2026-04-19（30天前） | 30 | ✅ 触发 | 偏好开启则发送 | 过期30天后也触发（因绝对值匹配） |
| **历史日期** | 2026-02-17（90天前） | 90 | ✅ 触发 | 偏好开启则发送 | 过期90天后也触发（因绝对值匹配） |
...
WarnAboutBills::handle()
    ↓
needsWarning(bill, field)
    ├─ 检查 bill->field 是否为 null → null 则返回 false
    ├─ 计算 diff = today->diffInDays(bill->field) → 返回绝对值
    └─ 检查 in_array(diff, [90, 30, 14, 7, 0]) → true 则继续
    ↓
sendWarning(bill, field)
    ↓
触发 SubscriptionNeedsExtensionOrRenewal 事件
    ↓
...
到期提醒**受 `notification_bill_reminder` 偏好开关控制**，但不会记录已提醒状态。只要 `diff` 在配置列表 `[90, 30, 14, 7, 0]` 中且偏好开关打开，每次 Cron 运行都会触发提醒。
...
**行为总结**：
- ✅ 如果用户启用提醒（默认true），每次满足条件时都会提醒（无去重）
- ❌ 如果用户关闭提醒，所有到期提醒都会被拦截
- ⚠️ 历史日期（过期后7/14/30/90天）也会触发提醒（因 `diffInDays` 返回绝对值）
- 🔄 依赖 Cron 12小时间隔控制，每天最多提醒2次
- 📝 通知标题区分：`diff=0` 时使用 "now" 主题，其他使用普通主题（`BillReminder.php:112-119`）
...
| 提醒类型 | 历史日期行为 | 当日行为 | 未来日期行为 | 触发机制 | 拦截/去重机制 |
|---------|-------------|---------|-------------|----------|--------------|
| **到期提醒** | ✅ 会触发（diff 绝对值匹配 `[90, 30, 14, 7, 0]`） | ✅ 会触发（diff=0） | ✅ 会触发（diff 匹配 `[90, 30, 14, 7, 0]`） | **配置周期命中就触发**：只要 diff 在配置列表中就触发事件 | `notification_bill_reminder` 偏好开关拦截（无去重，每次满足条件都提醒） |
| **逾期提醒** | ✅ 在当期范围内且 diff ≥ 6 触发 | ❌ 不触发（diff=0 < 6） | ⚠️ 在当期范围内且 diff ≥ 6 **会误触发**（因 diffInDays 返回绝对值）；不在当期范围内不触发 | **逾期阈值命中就触发**：未支付且最早应付款日距今 ≥ 6 天 | 哈希去重（基于 pay_dates 哈希），同一期账单只提醒一次 |

> **到期提醒核心机制**：配置周期命中就触发 + 偏好开关拦截  
> **逾期提醒注意事项**：对未来日期的误触发是潜在 Bug，原因是 `Carbon::diffInDays()` 返回绝对值，且代码中没有判断 `$earliest` 是否在过去。
...
3.  **到期提醒历史日期触发问题**：由于 `diffInDays()` 返回绝对值，导致历史日期（过期后7/14/30/90天）也会触发到期提醒，可能造成过度提醒
4.  **到期提醒无去重**：到期提醒没有去重机制，满足条件时每次运行都会提醒（依赖Cron间隔控制频率）
5.  **逾期提醒未来日期误触发**：`WarnAboutBills.php:168` 使用 `diffInDays()` 返回绝对值，且没有判断 `$earliest` 是否在过去，导致未来超过6天的应付款日期也会误触发逾期提醒
